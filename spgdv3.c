...
int main(void)
{
    ...
    inputSignalV = (float64*)malloc(1 * sizeof(float64));
    inputSignalPlus = (float64*)malloc(1 * sizeof(float64));
    inputSignalMinus = (float64*)malloc(1 * sizeof(float64));
    ...
    for (i = 0; i < nLoop; i++)
    {
        ...
        for (int j = 0; j < nChannels; j++)
        {
            gradient[j] = (inputSignalPlus - inputSignalMinus) * deltaV[j];
        }
